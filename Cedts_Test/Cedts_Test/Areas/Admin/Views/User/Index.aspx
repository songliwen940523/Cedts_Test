        ...
        function uploadexcel() {
            $("#uploadify").uploadify({
                ...
                'onComplete': function (event, queueId, fileObj, response, data) {
                    ...
                    $.post("/Admin/User/Excel", { filepath: response }, function (data) {
                        if (data.message != 1) {
                            $('#tt').dialog('close');
                            alert(data.url);
                        }
        ...
        <table class="tbRecords" cellspacing="0" >
            ...
            <tr>
                ...
                <td>
                    <%if (item.State != true)
                      { %>
                    <%:Html.ActionLink("同意", "Audit", new { id=item.UserID,State=true})%>
                    <%:Html.ActionLink("拒绝", "Audit", new { id=item.UserID,State=false})%>
                    <%} %>
                    <%else
                      { %>
                    <a href="#" id="<%:item.UserID %>" class="a">移除</a>
                    <a href="#" name="<%:item.UserID %>" class="b">换班</a>
                    <%} %>
                </td>
            </tr>
            <% } %>
...
{  
    ...
    ShowGoButton = false,PageIndexBoxWrapperFormatString=" 转到{0}",SeparatorHtml = "" 
        ...
        <br />
        <p>
            <%: Html.ActionLink("单个导入学生", "Create") %>&nbsp;<input type="button" value="Excel导入学生信息"
                id="btnE" />
        </p>
    </div>
    <%--隐藏层Html--%>
    ...
    <div id="chooseClass">
        <center class="classlist"></center>
        <center><input type="button" value="确定" class="changeclassbtn"/></center>
    </div>
    <div id="tt">
    </div>
    <input type="hidden" value='<%:ViewData["UserID"] %>' id="userid"/>
    <input type="hidden" value='<%:ViewData["ClassId"] %>' id="classid"/>
    <%--弹出层JS--%>
    <script type="text/javascript">
        $("#btnE").click(function () {
            $('#dd').show();
        ...
        $(function () {
            ...
            $(".a").click(function () {
                var id = $(this).attr("id");
                $.messager.confirm('提    示', '<b>你确认要移除该班级?<br>如果移除，与之相关的作业信息将全部丢失！</b>', function (r) {
                    if (r) {
            ...
            $(".b").click(function (e) {
                ...
                var pointX = e.pageX;
                var pointY = e.pageY;
                var offX = $("#chooseClass").parent().offset().left;
                var offY = $("#chooseClass").parent().offset().top;
                var studid = $(this).attr("name");
                $.post("/Admin/User/GetClassInfo", { UserID: userid }, function (data) {
                    var droplist = "移至班级：<select id = 'classdroplist'>";
                    for (var i = 0; i < data.length; i++) {
                        droplist = droplist + "<option value = '" + data[i]["ClassID"] + "'>" + data[i]["ClassName"] + "</option>";
                    }
                    droplist += "</select>";
                    $(".classlist").append(droplist);
                    $("#chooseClass").css("left", pointX - offX + "px");
                    $("#chooseClass").css("top", pointY - offY + 15 + "px");
                    $('#chooseClass').css("display", "block");
                    $(".changeclassbtn").attr("id", studid);
                }, "json")
            });
            $("#chooseClass").click(function (e) {
                e.stopPropagation();
            });
            $("html").click(function () {
                $("#chooseClass .changeclassbtn").removeAttr("id");
                $("#chooseClass .classlist").empty();
            ...
            $(".changeclassbtn").click(function () {
                ...
                var classid = $("#classdroplist").val();
                $.post("/Admin/User/ChangeClass", { StudID: studid, ClassID: classid }, function (data) {
                    if (data == "1") {
                        alert("更换成功！");
                        location.href = "/Admin/User/Index?ClassID=" + currentclassid;
                    }
                }, "json")
            })
        })
