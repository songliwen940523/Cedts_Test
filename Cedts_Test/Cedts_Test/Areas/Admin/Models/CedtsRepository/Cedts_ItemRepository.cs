...
namespace Cedts_Test.Areas.Admin.Models
{
    public class Cedts_ItemRepository : ICedts_ItemRepository
    {
        ...
        /// 新增阅读理解-信息匹配
        /// </summary>
        /// <param name="InfoM">组阅读理解-信息匹配XML文档信息</param>
        void ICedts_ItemRepository.AddInfMItem(InfoMatchingCompletion InfoM)
        {
            doc = new XmlDocument();
            //加入XML的声明段落
            node = doc.CreateNode(XmlNodeType.XmlDeclaration, "", "");
            ...
            //加入一个根元素
            elem = doc.CreateElement("", "AssessmentItem", "");
            elem.SetAttribute("ItemID", InfoM.Info.ItemID.ToString());
            elem.SetAttribute("PartType", InfoM.Info.PartType);
            elem.SetAttribute("ItemType", InfoM.Info.ItemType);
            elem.SetAttribute("ItemType_CN", InfoM.Info.ItemType_CN);
            elem.SetAttribute("Score", InfoM.Info.Score.ToString());
            elem.SetAttribute("ReplyTime", InfoM.Info.ReplyTime.ToString());
            elem.SetAttribute("Diffcult", InfoM.Info.Diffcult.ToString());
            elem.SetAttribute("QustionInterval", InfoM.Info.QustionInterval);
            elem.SetAttribute("Course", InfoM.Info.Course);
            elem.SetAttribute("Unit", InfoM.Info.Unit);
            doc.AppendChild(elem);
            ...
            XmlNode AssessmentItem = doc.SelectSingleNode("AssessmentItem");//查找<AssessmentItem> 
            XmlElement Content = doc.CreateElement("Content");
            Content.InnerText = InfoM.Content;
            AssessmentItem.AppendChild(Content);

            XmlElement WordList = doc.CreateElement("WordList");
            AssessmentItem.AppendChild(WordList);

            for (int i = 0; i < InfoM.WordList.Count; i++)
            {
                XmlElement Word = doc.CreateElement("Word");
                Word.SetAttribute("id", "A" + i);
                Word.InnerText = InfoM.WordList[i];
            ...
            XmlElement Questions = doc.CreateElement("Questions");
            Questions.SetAttribute("Count", InfoM.Info.QuestionCount.ToString());
            AssessmentItem.AppendChild(Questions);

            for (int i = 0; i < InfoM.Info.QuestionCount; i++)
            {
                XmlElement question = doc.CreateElement("question");
                question.SetAttribute("id", i.ToString());
                Questions.AppendChild(question);
                ...
                XmlElement value = doc.CreateElement("Value");
                value.InnerText = InfoM.Info.AnswerValue[i];
                Answer.AppendChild(value);

                XmlElement response = doc.CreateElement("Response");
                response.InnerText = InfoM.Info.AnswerResposn;
                Answer.AppendChild(response);

                XmlElement tip = doc.CreateElement("Tip");
                tip.InnerText = InfoM.Info.Tip[i];
                question.AppendChild(tip);
            }


            string path = AppDomain.CurrentDomain.BaseDirectory + "ExaminationItemLibrary\\" + InfoM.Info.ItemID + ".xml";
            //保存创建好的XML文档
            doc.Save(path);
        ...
        void ICedts_ItemRepository.EditInfoMatch(InfoMatchingCompletion InfoMatch)
        {
            doc = new XmlDocument();
            //加入XML的声明段落
            node = doc.CreateNode(XmlNodeType.XmlDeclaration, "", "");
            ...
            //加入一个根元素
            elem = doc.CreateElement("", "AssessmentItem", "");
            elem.SetAttribute("ItemID", InfoMatch.Info.ItemID.ToString());
            elem.SetAttribute("PartType", InfoMatch.Info.PartType);
            elem.SetAttribute("ItemType", InfoMatch.Info.ItemType);
            elem.SetAttribute("ItemType_CN", InfoMatch.Info.ItemType_CN);
            elem.SetAttribute("Score", InfoMatch.Info.Score.ToString());
            elem.SetAttribute("ReplyTime", InfoMatch.Info.ReplyTime.ToString());
            elem.SetAttribute("Diffcult", InfoMatch.Info.Diffcult.ToString());
            elem.SetAttribute("QustionInterval", InfoMatch.Info.QustionInterval);
            elem.SetAttribute("Course", InfoMatch.Info.Course);
            elem.SetAttribute("Unit", InfoMatch.Info.Unit);
            doc.AppendChild(elem);
            ...
            XmlNode AssessmentItem = doc.SelectSingleNode("AssessmentItem");//查找<AssessmentItem> 
            XmlElement Content = doc.CreateElement("Content");
            Content.InnerText = InfoMatch.Content;
            AssessmentItem.AppendChild(Content);

            XmlElement WordList = doc.CreateElement("WordList");
            AssessmentItem.AppendChild(WordList);

            for (int i = 0; i < InfoMatch.WordList.Count; i++)
            {
                XmlElement Word = doc.CreateElement("Word");
                Word.SetAttribute("id", "A" + i);
                Word.InnerText = InfoMatch.WordList[0];
                WordList.AppendChild(Word);
            }


            XmlElement Questions = doc.CreateElement("Questions");
            Questions.SetAttribute("Count", InfoMatch.Info.QuestionCount.ToString());
            AssessmentItem.AppendChild(Questions);

            for (int i = 0; i < InfoMatch.Info.QuestionCount; i++)
            {
                XmlElement question = doc.CreateElement("question");
                question.SetAttribute("id", i.ToString());
                Questions.AppendChild(question);
                ...
                XmlElement value = doc.CreateElement("Value");
                value.InnerText = InfoMatch.Info.AnswerValue[i];
                Answer.AppendChild(value);

                XmlElement response = doc.CreateElement("Response");
                response.InnerText = InfoMatch.Info.AnswerResposn;
                Answer.AppendChild(response);

                XmlElement tip = doc.CreateElement("Tip");
                tip.InnerText = InfoMatch.Info.Tip[i];
                question.AppendChild(tip);
            }


            string path = AppDomain.CurrentDomain.BaseDirectory + "ExaminationItemLibrary\\" + InfoMatch.Info.ItemID + ".xml";
            //保存创建好的XML文档
            doc.Save(path);
        }
        ...
        InfoMatchingCompletion ICedts_ItemRepository.SelectInfoMatch(Guid item)
        {
            var question = (from m in db.CEDTS_Question where m.AssessmentItemID == item orderby m.Order ascending select m).ToList();
            var assess = (from m in db.CEDTS_AssessmentItem where m.AssessmentItemID == item select m).First();
            var Itemtype = (from m in db.CEDTS_ItemType where m.ItemTypeID == assess.ItemTypeID select m).First();
            ...
            info.DifficultQuestion = new List<double>();
            info.ScoreQuestion = new List<double>();
            InfoMatchingCompletion Rpc = new InfoMatchingCompletion();
            info.Knowledge = new List<string>();
            info.KnowledgeID = new List<string>();
            info.TimeQuestion = new List<double>();
