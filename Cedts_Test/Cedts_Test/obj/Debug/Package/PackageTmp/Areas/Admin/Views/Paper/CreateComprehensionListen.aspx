...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
    <script type="text/javascript">
        ...
        $(function () {
            ...
            $(".selectnum").live("change", function selctChange() {
                ...
                for (var j = 1; j <= selectnum; j++) {
                    ...
                    if ($("#checkrole").val() == "Admin") {
                        div_2 += "知识点：<a href='#' class='classa' id='a" + tempnum + "_" + j + "'" + " >请选择知识点</a>";
                        div_2 += "<input type='text' id='txt" + tempnum + "_" + j + "'" + " name='txt" + tempnum + "_" + j + "'" + " class='txt' value='' readonly='readonly' />";
                        div_2 += "<input class='hidden' type='hidden' id='hidden" + tempnum + "_" + j + "'" + " name='hidden" + tempnum + "_" + j + "'" + " /><br />";
                    }
            ...
            //上一步时为元素赋值
            $.post("/admin/paper/GetComprehensionListen", null, function (data) {
                if (data == "1") { }
                else {
                    $("#filename").val("");
                    for (var i = 1; i <= data.length; i++) {
                        ...
                        for (var j = 1; j <= data[i - 1].Info.QuestionCount; j++) {
                            ...
                            if ($("#checkrole").val() == "Admin") {
                                $("#txt" + i + "_" + j).val(data[i - 1].Info.Knowledge[j - 1]);
                                $("#hidden" + i + "_" + j).val(data[i - 1].Info.KnowledgeID[j - 1]);
                            }
    ...
    <div>
        ...
        <div id="menu">
            ...
            =>
            <%:Html.ActionLink("试卷听力理解添加","CreateComprehensionListen") %>
        </div>
        ...
        <input type="hidden" id="hidden" name="hidden" value='<%:ViewData["短文听力理解"] %>' />
        ...
        <div>
            <div>
                <h3>
                    您当前选择了<%:ViewData["短文听力理解"]%>个题目，请选择相应的听力文件：</h3>
                ...
                $('#dd').dialog({
                    ...
                    buttons: [{
                        ...
                        handler: function () {
                            ...
                            var hidden = "#hidden" + aid.substring(aid.lastIndexOf("a") + 1);
                            $(txt).attr("readonly", "");
                            $(txt).val("");
                            $(hidden).val("");
                            $(txt).val(text);
