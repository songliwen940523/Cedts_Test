<%@ Control Language="C#" Inherits="System.Web.Mvc.ViewUserControl<IEnumerable<Cedts_Test.Areas.Admin.Models.KnowSide>>" %>
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data == 1) {
                    ...
                    alert("删除失败!请查看是否存在下级知识点，或则该知识点已被赋值，若想删除，请先删除被赋值的试题！");
                }
...
<table class="tbRecords">
    ...
    <tr>
        ...
        <td>
            <%: Html.ActionLink("编辑", "EditSide", new { id = item.KnowID })%>
        </td>
    </tr>
    <% } %>
</table>
<p class="actionbar">
    <%: Html.ActionLink("新增", "CreateSide") %><input type="button" id="Delete" value="删除"
        onclick="DeleteSide()" />
</p>
